...
handleWords=(text)=>{
  ...
  if(text && text.length > 0 && text !== " " && text !== "" && text !== "   "){
...
showMic=()=>{
  if(this.state.userSpeaking){
      return <Icon type="stop" className="watson-stop" onClick={this.handleTranscript}/>
  }else{
      return <Icon type="microphone" className="watson-mic" onClick={this.handleStart}/>
  }
}
...
export default SpeechRecognition(options)(connect(mapStateToProps)(PracticeSpeech))
