...
import {selectScrambled, unSelectScrambled} from '../../redux/action/categories'
...
class UnscrambleCard extends Component {
  ...
  isCorrect=(item,message)=>{
    ...
    let solution = item.solution.replace(regex,'').trim().toLowerCase()
  ...
  render() {
    return (
      ...
      {this.props.isSelected ?
        <Icon type="stop" className="watson-mic" onClick={(e)=>this.unSelectScrambled(e,this.props.item)}/>
         :
         <Icon type="microphone" className="watson-mic" onClick={(e)=>this.selectedScrambled(e,this.props.item)}/>
       }
...
const mapStateToProps=(state,ownProps)=>{
  return{isSelected: state.selectScrambled.id === ownProps.item.id, user: state.user}
}

const mapDispatchToProps=(dispatch)=>{

  return {selectScrambled:(e)=> dispatch(selectScrambled(e)), unSelectScrambled: (e)=>dispatch(unSelectScrambled(e))}
}

export default SpeechRecognition(options)(connect(mapStateToProps,mapDispatchToProps)(UnscrambleCard))
